...
public class ScanAreaBase : MonoBehaviour
{
    [SerializeField] string enemyTag = "EnemyBase";
    [SerializeField] string enemyBaseTag = "PlayerBase";
    ...
    private void Start() {
        militaryBase = GameObject.FindGameObjectWithTag(enemyTag);
    ...
    private void emitSearchRay()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out searchRayHit, rangeDistance))
        {
            ...
            if (searchRayHit.transform.gameObject.layer == LayerMask.NameToLayer(enemyBaseTag))
            {
                Debug.Log("Found player base at coordinates: " + (searchRayHit.point));
